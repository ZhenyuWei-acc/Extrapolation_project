...
f_train = generate_dataframe(f1_source, f1_background, f2_source, f2_background)

pt_source, pt_target, pt_background = generate_pt(num)
f1_source, f1_target,f1_background = generate_f1(pt_source, pt_target, pt_background)
f2_source, f2_target,f2_background = generate_f2(pt_source, pt_target, pt_background)
f_test = generate_dataframe(f1_target, f1_background,f2_target,f2_background)
...
keras_model.fit(X, train_y, epochs=50, batch_size=100, verbose=1)
...
keras_model_tt.fit(X, train_y, epochs=50, batch_size=100, verbose=1)

from sklearn.metrics import roc_curve
y_pred_keras = keras_model_tt.predict(X_test).ravel()
...
plt.plot([0, 1], [0, 1], 'k--')
plt.plot(fpr_keras, tpr_keras, label='source-target (area = {:.3f})'.format(auc_keras))
plt.plot(fpr_tt, tpr_tt, label='target-target (area = {:.3f})'.format(auc_tt))
