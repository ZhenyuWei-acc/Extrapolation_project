...
def generate_pt_reweighted(num: np.float64):
    np.random.seed(int(time.time()))
    pt_source = []
    for i in range(num):
        temp =  np.random.normal(loc=30,scale = 70,size = 1)
        if temp < 0:
            temp = -temp
        pt_source = np.append(pt_source,temp)
    pt_target = []
    for i in range(num):
        temp =  np.random.normal(loc=350,scale = 100,size = 1)
        if temp < 0:
            temp = -temp
        pt_target = np.append(pt_target,temp)
    pt_background = []
    for i in range(num_noi):
        temp =  np.random.exponential(scale = 80, size = 1)
        if temp < 0:
            temp = -temp
    ...
    Hpt_sy,edges = np.histogram(pt_source, bins=100, range=range_data,density = True )
    num_Hpt_sy,num_edges = np.histogram(pt_source, bins=100, range=range_data,density = False )
    Hpt_sx = (num_edges[1:] + num_edges[:-1]) / 2

    #plt.hist(pt_target, bins=100, range=range_data ,density = True,color ='b',histtype='step',label = 'pt_target')
    Hpt_ty,edges = np.histogram(pt_target, bins=100, range=range_data, density = True )
    num_Hpt_ty, num_edges = np.histogram(pt_target, bins=100, range=range_data, density = False )
    Hpt_tx = (num_edges[1:] + num_edges[:-1]) / 2

    plt.hist(pt_background, bins=100, range=range_data ,density = True,color ='g',histtype='step',label = 'pt_background')
    Hpt_by,edges = np.histogram(pt_background, bins=100, range=range_data,density = True )
    num_Hpt_by,num_edges = np.histogram(pt_background, bins=100, range=range_data, density = False )
    Hpt_bx = (num_edges[1:] + num_edges[:-1]) / 2
    plt.xlabel('X')
    plt.ylabel('Y')
    plt.legend()
    plt.show()
    for i in range(len(Hpt_by)):
        if Hpt_by[i] == 0.:
            Hpt_by[i] =5/num_noi
       
    ratio_s_b = np.divide(Hpt_sy , Hpt_by)
    print(ratio_s_b)
    ratio_t_b = np.divide(Hpt_ty , Hpt_by)
    print(ratio_t_b)
    for i in range(len(Hpt_bx)):
        if ratio_s_b[i] < 1:
            pt_source = np.append(pt_source,np.random.normal(loc = Hpt_sx[i], scale = abs((edges[1]-edges[0]))/2, size = round( -(num_Hpt_sy[i])*ratio_s_b[i] + (num_Hpt_sy[i])) ) ) 
        if ratio_t_b[i] < 1:
            pt_target = np.append(pt_target,np.random.normal(loc = Hpt_tx[i], scale = abs((edges[1]-edges[0]))/2, size = round( -(num_Hpt_ty[i])*ratio_t_b[i] + (num_Hpt_ty[i])) ) ) 
        

        if ratio_s_b[i] > 1:
            pt_source = np.sort(pt_source)
            pt_source_index = range(len(pt_source) - sum((pt_source>=num_edges[i]).astype(int)),  sum((pt_source<num_edges[i+1]).astype(int) ))
            del_num = round((ratio_s_b[i]-1) * num_Hpt_sy[i])
            pt_s_del_index = np.random.uniform(pt_source_index[0],pt_source_index[-1],del_num)
            pt_s_del_index = pt_s_del_index.astype(int)
            pt_source = np.delete(pt_source, pt_s_del_index)
            #pt_source = pt_source.sample(frac = 1).reset_index(drop=True) 

        if ratio_t_b[i] > 1:
            pt_target = np.sort(pt_target)
            pt_target_index = range(len(pt_target) - sum((pt_target>=num_edges[i]).astype(int)),   sum((pt_target<num_edges[i+1]).astype(int)) )
            del_num = round((ratio_t_b[i]-1) * num_Hpt_ty[i])
            pt_t_del_index = np.random.uniform(pt_target_index[0],pt_target_index[-1],del_num)
            pt_t_del_index = pt_t_del_index.astype(int)
            pt_target = np.delete(pt_target, pt_t_del_index)
            #pt_target = pt_target.sample(frac = 1).reset_index(drop=True)

    plt.hist(pt_source, bins=100, range=[0,1100],density = True ,color ='r',histtype='step',label = 'pt_source')
    #plt.hist(pt_target, bins=100, range=[0,1100] ,density = True,color ='b',histtype='step',label = 'pt_target')
    plt.hist(pt_background, bins=100, range=[0,1100] ,density = True,color ='g',histtype='step',label = 'pt_background')
    plt.xlabel('X')
    plt.ylabel('Y')
    plt.legend()
    plt.show()
...
def generate_dataframe(f_source:np.array,pt_source:np.array, f_background:np.array, pt_background:np.array):
    id = np.ones([len(f_source),1])
    id = id.astype(int)
    id = id.flatten()
    ...
    f_source = pd.DataFrame({'x': pd.Series(f_source)})
    ...
    f_background = pd.DataFrame({'x': pd.Series(f_background)})
    ...
    f = pd.DataFrame({'x':f['x'],'pt' : pt['pt'],'class_id': class_id['class_id']})
...
pt_source, pt_target, pt_background = generate_pt_reweighted(num)
...
f_train = generate_dataframe(f2_source,pt_source, f2_background, pt_background)

pt_source, pt_target, pt_background = generate_pt_reweighted(num)
f1_source, f1_target,f1_background = generate_f1(pt_source, pt_target, pt_background)
f2_source, f2_target,f2_background = generate_f2(pt_source, pt_target, pt_background)
f_test = generate_dataframe(f2_target, pt_target, f2_background, pt_background)
...
predict = pd.DataFrame({'x': predict['x'],'pt': predict['pt'],'class_ID':class_ID,'probability':probability,'fact':fact})
'''
predict = f2_test
fact = test_y
